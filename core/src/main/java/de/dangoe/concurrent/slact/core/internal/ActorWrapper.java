...
final class ActorWrapper<M> implements ActorHandle<M> {
  ...
  private class ActorContextImpl implements ActorContext<M> {
    ...
    public @NotNull Future<Done> stop(final @NotNull ActorHandle<?> actor) {
      return ((ActorWrapper<?>) actor).requestResponseToLifecycleControlInternal(
          new StopActorCommand(sender().path()), sender()).thenApply(unused -> Done.instance());
    }
  }
  ...
  private final class StartupActorLogic implements ActorLogic<M> {
    ...
    public boolean processMailboxItem(final @NotNull MailboxItem item,
      ...
      if (item instanceof CompleteStartActorCommand) {

        if (ActorWrapper.this.logger.isDebugEnabled()) {
          logger.debug("Received complete start command from '{}'.", sender.path());
        }
  ...
  private final class ReadyActorLogic implements ActorLogic<M> {
    ...
    public boolean processMailboxItem(final @NotNull MailboxItem item,
      ...
      switch (item) {
        case StopActorCommand unused -> {
          initiateStop(context);
          return true;
        }
        case ActorStoppedEvent unused -> {
          unregisterChild(item.sender());
          return true;
        }
  ...
  private final class StoppingActorLogic implements ActorLogic<M> {
      ...
      if (item instanceof TryCompleteStopActorCommand) {
        ...
        if (!children.isEmpty()) {
          ...
          if (!stopPropagated) {
            ...
            children.values().forEach(child -> child.sendLifecycleControlMessage(
    ...
    private void completeStop(final @NotNull ActorContext<M> context) {
      ...
      if (!context.self().path().isRoot()) {
        ...
        final var parentActor = (ActorWrapper<?>) context.parent();

        if (logger.isDebugEnabled()) {
          logger.debug("Sending stop notification to parent actor '{}'.", parentActor.path());
        }

        parentActor.sendLifecycleControlMessage(new ActorStoppedEvent(path()));
      }
  ...
  public ActorWrapper(final @NotNull ActorLogger logger, final @NotNull Actor<M> delegate,
      ...
      final @NotNull Consumer<ActorPath> unregisterActorFn,
    ...
    this.unregisterActorFn = unregisterActorFn;
  ...
  private void rescheduleMessageProcessing() {

    if (this.stopped.get()) {
      return;
    }

    scheduledExecutor.scheduleOnce(this::processMessages, Duration.ZERO);
  }
  ...
  private void initiateStop(final @NotNull ActorContext<M> actorContext) {
    ...
    this.setLogic(new StoppingActorLogic(actorContext.sender().path()));

    sendLifecycleControlMessage(new TryCompleteStopActorCommand(actorContext.self().path()));
  }

  void registerChild(final @NotNull ActorWrapper<?> childActor) {

    if (logger.isDebugEnabled()) {
      logger.debug("Registering child actor '{}'.", childActor.path());
    }

    this.children.put(childActor.path(), childActor);
  }

  private void unregisterChild(final @NotNull ActorPath childActorPath) {

    if (logger.isDebugEnabled()) {
      logger.debug("Unregistering child actor '{}'.", childActorPath);
    }

    children.remove(childActorPath);
    ActorWrapper.this.unregisterActorFn.accept(childActorPath);
  }
