...
public final class ContextMergeActor extends Actor<ContextMergeActor.Message> {
  ...
  public sealed interface Message permits Message.Merge {
     ...
     */
    record Merge(@NotNull String originalPrompt, @NotNull List<MemoryEntry> memories,
  ...
  public void onMessage(final @NotNull Message message) {
    switch (message) {
      case Message.Merge merge -> send((LlmCallActor.Message) new LlmCallActor.Message.Call(
          mergeContext(merge.originalPrompt(), merge.memories()), merge.replyTo())).to(
          merge.nextActor());
    }
  }
